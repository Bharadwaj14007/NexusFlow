...
export default function AutomationsView({ initialWorkflows }: { initialWorkflows: Workflow[] }) {
  ...
  return <div className="flex flex-col gap-6">
    ...
    {executions.length > 0 && <Card><h3 className="font-semibold">Execution history</h3><div className="mt-3 divide-y divide-border">{executions.map(execution => <div key={execution.id} className="flex items-center justify-between gap-3 py-2 text-xs"><span>{execution.workflow.name} · {execution.trigger}{execution.retryCount > 0 && <span className="ml-2 text-muted-foreground">retry {execution.retryCount}</span>}{execution.error && <span className="ml-2 text-destructive">{execution.error}</span>}</span><span className={execution.status === 'FAILED' ? 'text-destructive' : execution.status === 'COMPLETED' ? 'text-emerald-600' : 'text-amber-600'}>{execution.status}</span></div>)}</div></Card>}
  </div>
}
