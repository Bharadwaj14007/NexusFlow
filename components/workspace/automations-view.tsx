...
import { createWorkflowAction, deleteWorkflowAction, duplicateWorkflowAction, listWorkflowExecutionsAction, setWorkflowStatusAction, testWorkflowAction, updateWorkflowAction } from '@/app/actions/workflows'

type Workflow = { id: string; name: string; description: string; status: string; definition: Definition; _count: { executions: number } }
...
type Execution = { id: string; trigger: string; status: string; error: string | null; startedAt: string; workflow: { name: string } }
const triggers = ['TASK_CREATED', 'TASK_UPDATED', 'TASK_COMPLETED', 'TASK_OVERDUE', 'PROJECT_CREATED', 'COMMENT_ADDED']
...
export default function AutomationsView({ initialWorkflows }: { initialWorkflows: Workflow[] }) {
  ...
  const save = async (publish: boolean) => {
    ...
    setSelected(null); setName(''); setConditionValue(''); setMessage(publish ? 'Workflow published.' : 'Workflow saved as draft.')
  }
